...
/**
 * A simple hospital system with only one ward. Patients and doctors can be
 * created,
 * and patients assigned to a doctor and/or placed in a bed of the ward.
 */
public class HospitalSystem {
    /**
     * The interface to be used to read input from the user and output results to
     * the user.
     */
    private InputOutputInterface ioInterface;

    /**
     * Initialize the system by creating the dictionaries, ward, and interface for
     * I/O.
     */
    ...
    public void createWard() {
        ...
        CreateWard cmd = new CreateWard();
        cmd.createWard(name, firstBedLabel, lastBedLabel);
    ...
    public void systemState() {
        CurrentState cmd = new CurrentState();
        cmd.findCurState();
        if (cmd.wasSuccessful()) {
            ioInterface.outputString("The system is as follows: " + cmd.getCurState() + "\n");
        } else {
            throw new RuntimeException(cmd.getErrorMessage());
        }
    }
    ...
    public void displayEmptyBeds() {
        ...
        cmd.findEmptyBedsList();
        LinkedList<Integer> emptyBedsList = cmd.getEmptyBedsList();
    ...
    /**
     * Output the prompt that lists the possible tasks, and read the selection
     * chosen by the user.
     *
     * @return the int corresponding to the task selected
     */
    public int readOpId() {
        String[] taskChoices = new String[] { "quit", "add a new patient", "add a new doctor",
